...
def start_game():
  ...
  dealer = Positionable_Card(Card.Ranks[0], Card.Suit[0], True)
...
def start_game():
  ...
  for i, player_hand in enumerate(player_hands):
    for j, player_hand in enumerate(player_hands):
      total[i] += player_hand.cards[j].rank
    
  for i, player_hand in enumerate(player_hands):
     print(f"{players[i]}'s hand: {player_hand}", total[i])
